...
def load_best_hyperparameters(config):
    ...
    if os.path.exists(best_hyper_path):
        try:
            ...
            # Mise à jour des paramètres de trading
...
def run_backtest(config):
    ...
    # Récupération des paramètres de trading et gestion des risques depuis la config
    ...
    # Instanciation du RiskManager avec le risque maximal par trade
    ...
    for i in range(200, len(df)):
        ...
        # Ouverture de trade si aucune position n'est ouverte
        if position == 0:
